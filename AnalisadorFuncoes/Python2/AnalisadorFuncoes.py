...
def exibeMsgConfirmacao():
    ...
    while(True):
        resposta = str(raw_input("O(s) script(s) atende(m) às condições acima:[s/n] "))
...
class AnalisadorArvoreAST():
    ...
    def analisar(self):
        ...
        classificadorFuncoesND = ClassificadorFuncoesND(funcoesProcessar[:], buscadorFuncoesClassesMetodos.getModulosImportados()[:])
        ...
        classificadorFuncoesNPSC = ClassificadorFuncoesNPSC(funcoesProcessar[:],
    ...
    def classificaFuncoesQueChamamFuncoesND(self, grafoFuncoes):
        ...
        while(verticesNaoProcessados != []):
                ...
                for verticeLigado in verticeGrafoFuncoes.getVerticesLigados():
                    ...
                    for funcao in self.classificacaoFuncoesMetodos:
                        if(funcao.getDado() == verticeLigado.getDado() and
                            ...
                            funcao.setClassificacao("ND")
                            motivoClassificacao = "Essa funcao chama a funcao \"" + nomeFuncaoChamada + "\" que nao e deterministica"
            ...
            #ser analisados
            verticesNaoProcessados = novosVerticesNaoProcessados[:]
                ...
                for verticeLigado in verticeGrafoFuncoes.getVerticesLigados():
                    ...
                    for funcao in self.classificacaoFuncoesMetodos:
                        if(funcao.getDado() == verticeLigado.getDado() and
                            ...
                            funcao.setClassificacao("NPSC")
                            motivoClassificacao = "Essa funcao chama a funcao \"" + nomeFuncaoChamada + "\" que o analisador nao e capaz de classificar"
                            funcao.setMotivoClassificacao(motivoClassificacao)
    ...
    def visit_FunctionDef(self, node):
        if(self.analisandoFuncoes and self.funcaoAtual == node):
            ...
            if(len(node.args.args) == 0):
                self.funcoesNF[node] = "Essa funcao nao possui argumentos"
                return
            ...
            if(not returnEncontrado):
                self.funcoesNF[node] = "Essa funcao pode retornar None"
                return
        ...
        if(self.percorrendoAST):
            if(len(node.args) == 0):
                ...
                if(funcaoChamada in self.funcoesMetodosClasse):
                    self.funcoesNF[self.funcoesMetodosClasse[funcaoChamada]] = "Essa função e chamada sem que se passe nenhum argumento"
        self.generic_visit(node)
...
class ClassificadorFuncoesND(ast.NodeVisitor):
    ...
    def funcoesNDBibliotecasExternasChamadas(self, callNode):
        ...
        if(funcaoChamada != ""):
            motivoClassificacaoND = "Essa funcao chama a funcao " + funcaoChamada + " que nao e deterministica"
    ...
    def visit_Global(self, node):
        self.funcoesND[self.funcaoAtual] = "Essa funcao utiliza em seu processamento valores contidos em variaveis globais"
        self.generic_visit(node)
        ...
        if(self.attributeNodeNomeFuncao != node):
            ...
            for instImport in self.modulosImportados:
                ...
                for modulo in listaModulos:
                    ...
                    if(i == j):
                        achou = True
                        self.funcoesNPSC[self.funcaoAtual] = ("Essa funcao utiliza o atributo " + ".".join(partesNomeAtributo) + " do modulo " +
                                modulo.name + " que nao esta presente no banco deste analisador")
                        break
                ...
                for modulo in listaModulos:
                    ...
                    if(i == j):
                        ...
                        if(not self.funcaoChamadaDeterministica(".".join(partesNomeFuncaoChamada))):
                            self.funcoesNPSC[self.funcaoAtual] = ("Essa funcao chama a funcao " +
                                ".".join(partesNomeFuncaoChamada) + "() que nao esta presente no banco deste analisador")
                        break
...
class EscritorAST(ast.NodeTransformer):
    ...
    def escreveClassificacaoFuncoesAST(self):
        ...
        marcadorFuncaoNF = ast.Expr(value=ast.Str(
                s="A funcao logo abaixo nao funciona com o IntPy")) 
        
        metodosInstanciaNaoEscritos = self.metodosInstancia[:]
        listaAuxiliar = self.metodosInstancia[:]
        ...
        for classe in self.classes:
            for metodoInstancia in metodosInstanciaNaoEscritos:
                if(metodoInstancia in classe.body):
                    ...
                    for funcaoMetodo in self.classificacaoFuncoesMetodos:
                        if(funcaoMetodo.getDado() == metodoInstancia):
                            if(funcaoMetodo.getClassificacao() == "D"):
                                marcador = ast.Expr(value=ast.Str(
                                            s="apesar de ser deterministica"))
                            elif(funcaoMetodo.getClassificacao() == "ND"):
                                marcador = ast.Expr(value=ast.Str(
                                            s="e nao e deterministica"))
                            elif(funcaoMetodo.getClassificacao() == "NPSC"):
                                marcador = ast.Expr(value=ast.Str(
                                            s="e nao pode ser classificada por este analisador"))
                    ...
                    if(motivoClassificacao != ""):
                        classe.body.insert(indiceMetodoInstancia, ast.Expr(value=ast.Str(
                                                                s=motivoClassificacao)))
                    if(marcador != None):
            ...
            metodosInstanciaNaoEscritos = listaAuxiliar[:]

        #Escrevendo classificação das demais funções
        marcadorFuncaoND = ast.Expr(value=ast.Str(
                s="A funcao logo abaixo nao e deterministica"))
        marcadorFuncaoNPSC = ast.Expr(value=ast.Str(
                s="O IntPy nao e capaz de classificar a funcao abaixo"))
        ...
        for funcaoMetodo in self.classificacaoFuncoesMetodos:
            ...
            if(funcaoMetodo.getClassificacao() == "D"):
                ...
                if(funcaoMetodo.getDado() in self.arvoreAst.body):
                    ...
                    self.arvoreAst.body.insert(indiceFuncaoMetodo, ast.Expr(value=ast.Str(
                                                                s=funcaoMetodo.getMotivoClassificacao())))
                    ...
                    for superFuncao in self.superFuncoes:
                        if(funcaoMetodo.getDado() in superFuncao.body):
                            ...
                            superFuncao.body.insert(indiceFuncaoMetodo, ast.Expr(value=ast.Str(
                                                                s=funcaoMetodo.getMotivoClassificacao())))
                    ...
                    if(not funcaoEncontrada):
                        ...
                        for classe in self.classes:
                            if(funcaoMetodo.getDado() in classe.body):
                                ...
                                classe.body.insert(indiceFuncaoMetodo, ast.Expr(value=ast.Str(
                                                                    s=funcaoMetodo.getMotivoClassificacao())))
                        ...
                        if(not funcaoEncontrada):
                            ...
                            for superMetodo in self.superMetodos:
                                if(funcaoMetodo.getDado() in superMetodo.body):
                                    ...
                                    superMetodo.body.insert(indiceFuncaoMetodo, ast.Expr(value=ast.Str(
                                                                        s=funcaoMetodo.getMotivoClassificacao())))
    ...
    def escreveImportacaoModuloIntPy(self):
        ...
        self.arvoreAst.body.insert(1, ast.Expr(value=ast.Call(func=comandoSysPathAppend,
                                                        args=[ast.Str(s=pathRepositorio)],
                                                        keywords=[])))
